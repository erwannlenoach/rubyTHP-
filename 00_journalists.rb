...
puts " il y a #{journalists.length} adresses e-mail."

#Quel est le handle le plus court de cette liste ?

puts "#{journalists.min_by(&:length)} est la plus courte adresse e-mail."

#Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)

puts journalists.count {|i| i.length == 6}

#Combien commencent par une majuscule (première lettre juste après le @) ?

puts journalists.count {|i| i[1,1] =~ /[A-Z]/}
...
puts journalists.sort { |a,b|  b.length - a.length}

#Quelle est la position dans l'array de la personne @epenser ?

puts journalists.index {|i| i == '@epenser'}
...
puts journalists.count {|i|i.length}
